<?php
/**
 * Family Detailed Report - CSV Exporter
 */
...
if (!$id) {
    header('Location: families.php?error=not_found');
    exit();
}
...
header('Content-Disposition: attachment; filename=family_report_' . strtolower(str_replace(' ', '_', $m['name'])) . '_' . date('Ymd_His') . '.csv');
...
// Write Family Overview Section
fputcsv($output, ['MINISTRY PROFILE REPORT']);
fputcsv($output, ['Family Name', $m['name']]);
fputcsv($output, ['Description', $m['description']]);
fputcsv($output, ['Family Head', $m['head_name'] ? $m['head_name'] . ' (' . $m['head_code'] . ')' : 'N/A']);
